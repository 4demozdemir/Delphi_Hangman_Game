...
var
  ...
  //Global deðiþkenler
  alphButtons : array[0..31] of TButton; //Harf buttonlarý
  keyLabels   : array[0..31] of TLabel;  //Ýpucu kelime labelleri
  pictHang    : Integer;                 //Resim sira tutucu
  hangWord    : string;                  //Tutulan sözcük
  gameStart   : Boolean;                 //oyun baþlangýç, bitiþ
  count       : Integer;                 //ipucu kelime sayacý
...
PaintPicture(Sender);
count :=0;
//Aktif olmayan butonlarý aktif et
for i:=0 to 31 do
...
Harf:=TButton(Sender).Caption;//Basýlan Butonun gösterdiði harfin alýnmasý

TButton(Sender).Enabled:=False;//Basýlan butonun Pasif hale getirilmesi

Found:=False;
for Yeri:=1 to Length(hangWord) do//Gelen harfin sözcük içinde aranmasý
 begin
  if hangWord[Yeri]=Harf Then//eðer gelen harf sözcük içinde varsa
   begin
    ...
    Found:=True;//Harfin sözcüðün içinde var olduðunun belirlenmesi
   end;
 end;

if not Found Then //Harfin sözcük içinde var olmamasý durumu
   begin
    Inc(pictHang);
    PaintPicture(Sender);
...
if pictHang = 9 then //Oyunun Bitmesi
   begin
    ...
    SpeedButton33.Enabled:=True;
    Panel3.Caption:='Aranan Sözcük : ' + hangWord;
    gameStart := False;
   end;
end;
...
//Sözcüklerin yeraldýðý tablonun açýlmasý
...
//Harf butonlarýnýn yapýlandýrýlmasý
...
begin
alphabet := 'ABCÇDEFGÐHIÝJKLMNOÖPQRSÞTUÜVWXYZ';
for i:=0 to 31 do
 begin
